odoo.define('aspl_pos_report.web', function (require) {
"use strict";
...
var QWeb = core.qweb;
		...
		on_confirmed: function() {
			...
			if (form_values['active_model'] == 'wizard.sales.details' && this.$el.hasClass('print_sales_details')){
				if (form_values['report_type'] == 'thermal' && form_values['proxy_ip']) {
					...
					new Model("report").call('get_html', [form_values['active_ids'], 'aspl_pos_report.sales_details_template']).done(
	...
	//                                        self.receipt_queue.unshift(r);
					...
					var print = {'context': {'active_id': form_values['active_id'],
						...
						'report_file': 'aspl_pos_report.sales_details_pdf_template',
						'report_name': 'aspl_pos_report.sales_details_pdf_template',
				...
				if (form_values['report_type'] == 'thermal') {
					...
					.then(function (res){
						if(res && res.ip) {
							...
							new Model("report").call('get_html', [form_values['session_ids'][0][2], 'aspl_pos_report.pos_sales_report_template']).done(
								function(report_html) {
									// WORKING  CODE FOR PRINTING VIA PROXY
									self.connection.rpc('/hw_proxy/print_xml_receipt',{receipt: report_html},{timeout: 8000})
	                                    .then(function(){
	...
	//                                            self.receipt_queue.unshift(r);
					...
					var print = {'context': {'active_id': form_values['session_ids'][0][2][0],
						...
						'report_file': 'aspl_pos_report.pos_sales_report_pdf_template',
						'report_name': 'aspl_pos_report.pos_sales_report_pdf_template',
						'report_type': "qweb-pdf",
						'type': "ir.actions.report.xml"};
					var options = {};
					var action_manager = new ActionManager();
				...
				if (form_values['report_type'] == 'thermal') {
					...
					.then(function (res){
						if(res && res.ip) {
							var url = res.ip;
							self.connection = new Session(undefined,url, { use_cors: true});
							new Model("report").call('get_html', [form_values['session_ids'][0][2], 'aspl_pos_report.front_sales_thermal_report_template']).done(
								function(report_html) {
									// WORKING  CODE FOR PRINTING VIA PROXY
									self.connection.rpc('/hw_proxy/print_xml_receipt',{receipt: report_html},{timeout: 8000})
	                                    .then(function(){
	...
	//                                            self.receipt_queue.unshift(r);
					...
					var print = {'context': {'active_id': form_values['session_ids'][0][2][0],
						'active_ids': form_values['session_ids'][0][2]},
						'report_file': 'aspl_pos_report.front_sales_report_pdf_template',
						'report_name': 'aspl_pos_report.front_sales_report_pdf_template',
						'report_type': "qweb-pdf",
						'type': "ir.actions.report.xml"};
					var options = {};
					var action_manager = new ActionManager();
