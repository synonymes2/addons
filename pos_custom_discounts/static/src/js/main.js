...
"use strict";
	...
	pos_model.Orderline = pos_model.Orderline.extend({
		...
		set_discount_id: function(discount_id){
			this.discount_id = discount_id;
		},
		get_discount_id: function(){
		...
		export_as_JSON: function() {
			...
			loaded.discount_id=self.get_discount_id();
			return loaded;
		}
	});
	...
	var WkCustomDiscountPopup = popup_widget.extend({
		...
		show: function(){
			...
			$('.current_product').on('click',function(){
				if (($('#discount').val())>100 || $('#discount').val()<0 || $('#discount').val() ==''){
					$('#error_div').show();
	...
	var WkDiscountPopup = popup_widget.extend({
		...
		show: function() {
			...
			$(".wk_product_discount").on("click",function(e){
				...
				discount_id = parseInt($(this).attr('id'));
			...
			$(".button.apply").on('click',function(){
				if(wk_discount_percentage != 0){
					...
					order.get_selected_orderline().set_discount_id(discount_id);
			...
			$(".button.apply_complete_order").on('click',function(){
				if(wk_discount_percentage != 0){
					...
					for(var i=0; i< orderline_ids.length; i++){
							...
							orderline_ids[i].set_discount_id(discount_id);
						}
